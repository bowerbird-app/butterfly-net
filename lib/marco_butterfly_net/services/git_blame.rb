  ...
  module Services
    ...
    class GitBlame
      ...
      def initialize(repo_path: nil)
        @repo_path = repo_path || MarcoButterflyNet.configuration.repo_path || default_repo_path
      end
      ...
      def run_git_blame(relative_path, line_number)
        ...
        Rails.logger.error("[MarcoButterflyNet] GitBlame error: #{e.message}") if defined?(Rails)
        nil
      end
