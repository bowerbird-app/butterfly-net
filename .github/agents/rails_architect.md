...
**Engine Isolation**: Respect namespace isolation. All code should be properly namespaced under the engine's module (`MarcoButterflyNet`). Use `isolate_namespace` patterns correctly.
...
**Engine-Specific**:
- Prefix all database tables with the engine name (e.g., `marco_butterfly_net_error_logs`)
...
**Version Management**:
- Update `lib/marco_butterfly_net/version.rb` following semantic versioning
...
When generating code, include file paths in comments (e.g., `# app/models/marco_butterfly_net/error_log.rb`).
...
**Configuration**:
- Provide a configuration block: `MarcoButterflyNet.configure do |config|`
